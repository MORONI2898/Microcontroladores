...
int main(){
	...
	if (strcmp(txt,txt2) ==0){
		...
		sprintf(txt,"Lapalabra introducida es correcta");	//sprintf, guarda texto y variables
															//en un string
		printf("%s",txt);//imprime
	
	} else{
		
		sprintf(txt,"Lapalabra introducida es incorrecta");	//sprintf, guarda texto y variables
															//en un string
		printf("%s",txt);//imprime
	}
